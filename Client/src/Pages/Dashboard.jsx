...
//           return false;
//         }
//         const matchesSearch = wo.title.toLowerCase().includes(query.toLowerCase()) ||
//                              (wo.description && wo.description.toLowerCase().includes(query.toLowerCase()));
...
//         // User/Assignee filter (multi-select)
...
export default function Dashboard() {
  ...
  const filtered = useMemo(() => {
    return workOrders
      .filter((wo) => {
        // Search filter - only search in title
        if (!wo || !wo.title || typeof wo.title !== 'string') {
          return false;
        }
