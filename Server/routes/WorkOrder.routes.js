...
import express from "express";
import { upload } from "../config/cloudinary.js";
import { compressUploadedImages } from "../middleware/imageCompression.middleware.js";
import {
  createWorkOrder,
  getAllWorkOrders,
  getWorkOrderById,
...
// GET routes (no file upload)
router.get("/stats", getWorkOrderStats);
router.get("/date-range", getWorkOrdersByDateRange);
router.get("/:id", getWorkOrderById);
router.get("/", getAllWorkOrders);

// POST routes with image upload and compression
router.post(
  "/",
...
// PUT routes with image upload and compression
router.put(
  "/:id",
  upload.array("photos", 5),
  compressUploadedImages,
