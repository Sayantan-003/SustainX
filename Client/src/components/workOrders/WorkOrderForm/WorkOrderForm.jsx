...
export default function WorkOrderForm({ onSubmit, onClose }) {
  ...
  const [checklists, setChecklists] = useState([]); // Start with empty array
  const [isSubmitted, setIsSubmitted] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState("");
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      ...
      // Filter out checklists with no tasks or empty task names
      ...
      // Only append checklists if there are valid ones
      if (validChecklists.length > 0) {
        formData.append("checklists", JSON.stringify(validChecklists));
      }
